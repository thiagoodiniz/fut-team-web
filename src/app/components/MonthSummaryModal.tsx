...
export function MonthSummaryModal({
      ...
      {data.map((rank, index) => (
        <div
          ...
          style={{
            ...
            padding: '12px 0',
            borderBottom:
              index < data.length - 1 ? `1px solid ${token.colorFillQuaternary}` : 'none',
          }}
        >
          <span
            style={{
  ...
  return (
    ...
    >
      <div
        style={{ display: 'flex', flexDirection: 'column', gap: 24, padding: '8px 0' }}
      >
        {/* Stats block */}
        <div
          style={{ background: token.colorFillQuaternary, padding: 16, borderRadius: 12 }}
        >
          <div
            style={{
              display: 'grid',
